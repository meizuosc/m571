...
#ifdef MEIZU_M81
        /* M81 will Support ATL and SONY battery, ATL:0; SONY:1*/
    #ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
    /* Cmax */
        kal_int32 g_Q_MAX_POS_50[] = {2956, 2846,2917}; /* {3096, 2980,3055};  */
        kal_int32 g_Q_MAX_POS_25[] = {2928, 2802,2949};/* {3066, 2935,3088}; */
        kal_int32 g_Q_MAX_POS_0[] = {2674, 2595,2758};/* {2800, 2718,2888}; */
        kal_int32 g_Q_MAX_NEG_10[] = {2317, 2299,2612};/*  {2427, 2408,2736}; */

    /* Cmax 400*/
        kal_int32 g_Q_MAX_POS_50_H_CURRENT[] = {3028, 2913,2977};
        kal_int32 g_Q_MAX_POS_25_H_CURRENT[] = {2954, 2823,2951};
        kal_int32 g_Q_MAX_POS_0_H_CURRENT[] = {2318, 2390,2137};
        kal_int32 g_Q_MAX_NEG_10_H_CURRENT[] = {817, 1195,790};
    #endif
#endif
...
int get_bat_id_volt(int times)
{
        int ret = 0;

        ret = PMIC_IMM_GetOneChannelValue(MT6328_AUX_TSX,times,1);  //Todo :   check  MT6328_AUX_TSX 
        printk(KERN_EMERG "\n++++++++++++PMIC_IMM_GetOneChannelValue: AUX_ADCVIN0_MD= %d\n\n", ret);

        return ret;
}

 /*
       SONY BATTERY THE BADID IS 1, PULL DOWN RESISTOR 200K;
       STL BATTERY BATID IS 0, PULL DOWN RESISTOR 49.9K,
       GY BATTERY BATID IS 2, PULL DOWN RESISTOR 100K,
       PULL UP  RESISTOR 100K
  */
#define V_BAT_ID_MAX 1800
#define SONY_ADC_VOLT 1200
#define ATL_ADC_VOLT 600
#define RANGE 100
#define GY_CDA_VOLT 900
#define bat_id_is(v,a,r)  ((((a)>(v)-(r))&&((a)<(v)+(r)))?true:false)

int  get_bat_id()
 {
    int adc_volt = 0;

    adc_volt = get_bat_id_volt(3);

    if(bat_id_is(SONY_ADC_VOLT,adc_volt,RANGE))            //SONY battery judge
    {
        pr_info("get_bat_id:  The battery is :  SONY battery ! \n");
        return BAT_SONY;
    }
    else if(bat_id_is(ATL_ADC_VOLT,adc_volt,RANGE))       //ATL battery judge
    {
        pr_info("get_bat_id:  The battery is :  ATL battery  !\n");
        return BAT_ATL;
    }
    else if(bat_id_is(GY_CDA_VOLT,adc_volt,RANGE))       //GY battery judge
    {
        pr_info("get_bat_id_type:  The battery is :  GY battery  !\n");
        return BAT_GY;
    }
    else            // NOT SUPPORT
    {
        if(adc_volt > (V_BAT_ID_MAX-RANGE))
            pr_err("get_bat_id:  The battery  may be false battery !!! \n");
        else
            pr_err("get_bat_id:  The battery can't support !!!! \n");
...
void fgauge_get_profile_id(void)
{
    ...
    g_fg_battery_id = get_bat_id();


}

#endif //MTK_MULTI_BAT_PROFILE_SUPPORT
#else //MEIZU_M81
#ifdef MTK_MULTI_BAT_PROFILE_SUPPORT
extern int IMM_GetOneChannelValue_Cali(int Channel, int *voltage);
kal_uint32 g_fg_battery_id = 0;
...
void fgauge_construct_battery_profile_init(void)
{
	...
	for (i=0; i< PROFILE_SIZE; i++) {
		...
		for (j=0; j*2 <= 100; j++) {
			...
			/* 2015-4-4 by Yin ShunQing, in order to close log print start */
			...
			#endif
			/* 2015-4-4 by Yin ShunQing, in order to close log print end */
		}
...
#ifdef MEIZU_M81
    g_rtc_fg_soc = 0;  //D0 do't get value from RTC
#endif
...
#ifdef MEIZU_M81
kal_int32 battery_meter_get_battery_current_now(void)
{
    int ret=0;
    int val=10; //set avg times
...
int  get_bat_id_type()
 {
    ...
    if(bat_id_is(SONY_ADC_VOLT,adc_volt,RANGE))            //SONY battery judge
    {
        pr_info("get_bat_id_type:  The battery is :  SONY battery ! \n");
        return BAT_SONY;
    }
    else if(bat_id_is(ATL_ADC_VOLT,adc_volt,RANGE))       //ATL battery judge
    {
        pr_info("get_bat_id_type:  The battery is :  ATL battery  !\n");
        return BAT_ATL;
    }
    else if(bat_id_is(GY_CDA_VOLT,adc_volt,RANGE))       //GY battery judge
    {
    ...
    {
        if(adc_volt > (V_BAT_ID_MAX-RANGE))
            pr_err("get_bat_id_type:  The battery  may be false battery !!! \n");
        else
            pr_err("get_bat_id_type:  The battery can't support !!!! \n");

        return BAT_ATL;
    }
}
static ssize_t show_FG_g_fg_dbg_bat_type(struct device *dev,struct device_attribute *attr, char *buf)
{  int id = 0;
    static char *status_text[] = {
        "ATL battery", "SONY battery", "GY battery"
    };

    id = get_bat_id_type();

    printk(KERN_EMERG "[FG] g_fg_dbg_battery_type : %d\n", id );

   return sprintf(buf, "%s\n", status_text[id]);;

}
static ssize_t store_FG_g_fg_dbg_bat_type(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
    return size;
}
static DEVICE_ATTR(FG_g_fg_dbg_bat_type, 0664, show_FG_g_fg_dbg_bat_type, store_FG_g_fg_dbg_bat_type);
...
#ifdef MEIZU_M81
    ret_device_file = device_create_file(&(dev->dev), &dev_attr_FG_g_fg_dbg_bat_type);
#endif
