...
#ifdef MEIZU_M81
extern kal_uint32 set_bat_charging_current_limit(int current_limit);
#endif
...
#ifdef MEIZU_M81
extern void max77819_aicl_control(void);
extern void max77819_vol_control(void);
#endif

void bat_vol_set_current(void)
{	kal_uint32 charger_voltage = 0, sum_voltage = 0,i;
			//msleep(50);
			for (i=0; i<5; i++) {
  ...
  printk(KERN_EMERG "@@@@@@@@@@@bat_vol_set_current : %d\n",charger_voltage);

            if(charger_voltage < 4430)
            {
                /*
                    g_temp_input_CC_value = AC_CHARGER_CURRENT;
	...
	/* enable/disable charging */
  	battery_charging_control(CHARGING_CMD_ENABLE,&charging_enable);
		...
		if (get_usb_current_unlimited()) {
            ...
            g_temp_CC_value = STANDARD_CHARGER_CURRENT;
                        if (g_bcct_flag == 1) {
                g_temp_input_CC_value = MED12C_CHARGER_CURRENT;
                g_temp_CC_value = MED12C_CHARGER_CURRENT;
                g_bcct_flag = 0;
            }
			#else
            g_temp_input_CC_value = AC_CHARGER_CURRENT;
