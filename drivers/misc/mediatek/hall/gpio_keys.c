...
static int key_hall_press = REMOVED;
#endif

#ifdef MEIZU_TP_NOTIFIER
static BLOCKING_NOTIFIER_HEAD(gpio_key_chain);

int register_gpio_key_notifier(struct notifier_block *nb)
{
        return blocking_notifier_chain_register(&gpio_key_chain, nb);
}
EXPORT_SYMBOL(register_gpio_key_notifier);

void unregister_gpio_key_notifier(struct notifier_block *nb)
{
        blocking_notifier_chain_unregister(&gpio_key_chain, nb);
}
EXPORT_SYMBOL(unregister_gpio_key_notifier);

static int gpio_key_notify(unsigned long val, void *v)
{
	return blocking_notifier_call_chain(&gpio_key_chain, val, v);
}
#endif //#ifdef MEIZU_TP_NOTIFIER
...
#ifdef KEY_HALL_DEBUG
static ssize_t key_hall_press_show(struct device *dev,struct device_attribute *attr, char *buf, size_t size)
{
	#ifdef KEYPAD_DEBUG
	printk("[Hall] %s: buf is %s and size is %zu \n", __func__, buf, size);
	#endif
					 ...
					 const struct gpio_keys_button *button)
{
	...
	setup_timer(&bdata->timer,
		...
		if(bdata->irq != M7X_HALL_IRQ_NUM) {
			...
			input_set_capability(input, EV_SW, button->code);
		}
...
{
#ifdef KEYPAD_DEBUG
	printk("[Hall] %s: register gpio-keys device and driver.\n", __func__);
#endif
	/* register m7x keyboard devices */
	m7x_gpio_keys_init();
