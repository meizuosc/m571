...
if (bl_last_level != 0)
{
    bl_abs_diff = level_1024 - bl_last_level;
    if (bl_abs_diff < 0)
    {
        bl_abs_flag = 1;
        bl_abs_diff = -bl_abs_diff;
    }
}
...
else
{
    do
    {
        //printk(KERN_EMERG"disp_pwm_set_backlight level=%d bl_abs_diff = %d \n",bl_last_level,bl_abs_diff);
        if (bl_abs_flag == 1)
        {
            bl_last_level--;
        }
        ...
        ret = disp_pwm_set_backlight_cmdq(id, bl_last_level, NULL);
        bl_abs_diff--;
    }while(bl_abs_diff > 0);
    //ret = disp_pwm_set_backlight_cmdq(id, level_1024, NULL);
}
