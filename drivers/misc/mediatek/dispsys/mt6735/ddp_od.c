...
static void _od_set_dram_buffer_addr(void *cmdq, int manual_comp, int image_width, int image_height)
{
    ...
    if (!is_inited)
    {
        ...
        va = dma_alloc_coherent(NULL, u4ODDramSize + OD_ADDITIONAL_BUFFER + OD_GUARD_PATTERN_SIZE, &dma_addr, GFP_KERNEL);
...
static void disp_od_set_enabled(unsigned long user_arg, void *cmdq)
{
    int enabled = 0;
    ...
    enabled &= 0x1;
    DISP_REG_MASK(cmdq, DISP_OD_CFG, (enabled << 1), 0x2);
}
