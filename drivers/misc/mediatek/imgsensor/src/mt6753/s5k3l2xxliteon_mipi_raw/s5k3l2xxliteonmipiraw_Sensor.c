...
#include "s5k3l2xxliteonmipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "S5K3L2XXLITEON_Largen_camera_sensor"
#define LOG_1 LOG_INF("S5K3L2XXLITEON_Largen,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = S5K3L2XXLITEON_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
#define LITEON_MODULE_ID   0x01
#define LSC_START_ADDR	   0x1C
...
typedef struct {
	...
	u8 af_direction;    //0x00:upward  0x01:horizontal
	u16 af_infinity;
	u16 af_macro;
}af_struct;
...
static liteon_otp_struct *liteon_otp_data;
u8 liteon_lsc_data[LITEON_LSC_SIZE];
...
static void read_otp_byte(u8 addr, u8 *buf,u8 write_id)
{
	...
	if (ret < 0)
		printk("read data from liteon otp e2prom failed! error reg addr: 0x%X\n",pu_send_cmd[0]);
}
...
static void read_lsc_data(void){
	...
	for(ptr_addr = LSC_START_ADDR;ptr_addr < LSC_START_ADDR+LITEON_LSC_SIZE;ptr_addr++){
		addr = ptr_addr%256;
		if( addr == 0) 
			write_id+=2;
		read_otp_byte(addr,&liteon_lsc_data[ptr_addr-LSC_START_ADDR],write_id);
	}
}

static void read_otp_data(liteon_otp_struct *pOtp)
{
	u8 addr = 0;
	u8 low = 0;
	u8 high = 0;
	...
	//base info	
	read_otp_byte(addr+3,&pOtp->sensor_id,write_id);
	read_otp_byte(addr+4,&pOtp->lens_id,write_id);
	read_otp_byte(addr+5,&pOtp->vcm_id,write_id);
	read_otp_byte(addr+6,&pOtp->vcm_driver_ic,write_id);
	read_otp_byte(addr+7,&pOtp->IR_id,write_id);
	read_otp_byte(addr+8,&pOtp->AF_flag,write_id);
	read_otp_byte(addr+9,&pOtp->module_id,write_id);	
	//AWB data		
	read_otp_byte(addr+10,&high,write_id); 
	read_otp_byte(addr+11,&low,write_id);
	pOtp->otp_awb->R_Gr = high<<8|low;
	read_otp_byte(addr+12,&high,write_id); 
	read_otp_byte(addr+13,&low,write_id);
	pOtp->otp_awb->B_Gb = high<<8|low;
	read_otp_byte(addr+14,&high,write_id); 
	read_otp_byte(addr+15,&low,write_id);
	pOtp->otp_awb->Gr_Gb = high<<8|low;
	read_otp_byte(addr+16,&high,write_id); 
	read_otp_byte(addr+17,&low,write_id);
	pOtp->otp_awb->golden_R_Gr = high<<8|low;
	read_otp_byte(addr+18,&high,write_id); 
	read_otp_byte(addr+19,&low,write_id);
	pOtp->otp_awb->golden_B_Gb = high<<8|low;
	read_otp_byte(addr+20,&high,write_id); 
	read_otp_byte(addr+21,&low,write_id);
	pOtp->otp_awb->golden_Gr_Gb = high<<8|low;
	//AF data
	read_otp_byte(addr+22,&pOtp->otp_af->underspin_flag,write_id); 
	read_otp_byte(addr+23,&pOtp->otp_af->af_direction,write_id); //read AF flag ,0x00:empty 0x01:invalid 0x11:valid
	read_otp_byte(addr+24,&high,write_id); 
	read_otp_byte(addr+25,&low,write_id);
	pOtp->otp_af->af_infinity = high<<8|low;
	af_inf_pos = pOtp->otp_af->af_infinity;
	read_otp_byte(addr+26,&high,write_id);
	read_otp_byte(addr+27,&low,write_id); 
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    kal_uint8 program_id = 0; //add this id to distinguish module
    kal_uint8 lens_id = 0;
    kal_uint32 tmp = 0;
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		read_otp_byte(0x09,&module_id,E2PROM_WRITE_ID);  //it might be liteon module,however,primax module's 0x09 might equal to 0x01
		if(module_id == LITEON_MODULE_ID){
			read_otp_byte(0x02,&program_id,E2PROM_WRITE_ID);  //read 0x02 ,if 0x4D,it should be primax module,not liteon
			if(program_id == 0x4D){
				*sensor_id = 0xFFFFFFFF;
			}
			else{
			LOG_INF("i2c write id: 0x%x, sensor id: 0x%x,liteon module id:%x\n", imgsensor.i2c_write_id,tmp,module_id);
			#if OTP_DATA_READ
			//alloc memory for otp_data struct
			liteon_otp_data = (liteon_otp_struct *)kzalloc(sizeof(liteon_otp_struct),GFP_KERNEL);
			liteon_otp_data->otp_awb = (awb_struct *)kzalloc(sizeof(awb_struct),GFP_KERNEL);
			liteon_otp_data->otp_af = (af_struct *)kzalloc(sizeof(af_struct),GFP_KERNEL);
			read_otp_data(liteon_otp_data);
			#endif
			read_lsc_data();    
			read_otp_byte(0x04,&lens_id,E2PROM_WRITE_ID);  //lens id,0x02:Largen  0x03:geso
			if(0x02 == lens_id){
				*sensor_id = imgsensor_info.sensor_id;
				platform_match = 1;
				back_cam_id = S5K3L2XXLITEON_SENSOR_ID;
				return ERROR_NONE;
			}
			else{
				*sensor_id = 0xFFFFFFFF;
    ...
    if (*sensor_id != imgsensor_info.sensor_id || module_id != LITEON_MODULE_ID || lens_id != 0x02) {
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == S5K3L2XX_SENSOR_ID && back_cam_id == S5K3L2XXLITEON_SENSOR_ID) {
                LOG_INF("s5k3l2xx liteon module, Largen lens, i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
...
UINT32 S5K3L2XXLITEON_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
