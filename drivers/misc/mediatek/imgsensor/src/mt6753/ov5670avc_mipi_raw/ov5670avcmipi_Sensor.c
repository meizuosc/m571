...
#include "ov5670avcmipi_Sensor.h"
#include "meizu_otp.h"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670AVCMIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
avc_otp_struct *avc_otp_data;

static int read_otp(avc_otp_struct *pOtp)
{
	...
	if(addr != 0) {
		...
		pOtp->production_year = read_cmos_sensor( addr + 2);
		pOtp->production_month = read_cmos_sensor( addr + 3);
		pOtp->production_day = read_cmos_sensor( addr + 4);
	}
	else {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == 0x5670) {				
				...
				if(NULL == avc_otp_data) {
				        LOG_INF("allocate mem failed\n");
					return -ENOMEM;
				}
				int flag = read_otp(avc_otp_data);
				if (avc_otp_data->module_id == AVC_MODULE_ID_A || avc_otp_data->module_id == AVC_MODULE_ID_B)   //this driver is for avc module
				{	
					LOG_INF("i2c write id: 0x%x, sensor id: 0x%x,module id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id,avc_otp_data->module_id);	  
					#if OTP_DATA_LOG 
					printk("[ov5670]read otp data flag =0x%x\n",avc_otp_data->flag);
					printk("[ov5670]read otp data module id =0x%x\n",avc_otp_data->module_id);
					printk("[ov5670]read otp data lens id =0x%x\n",avc_otp_data->lens_id);
					printk("[ov5670]read otp data AWB_RG_MSB=0x%x\n",avc_otp_data->awb_rg_msb);
				   	printk("[ov5670]read otp data AWB_BG_MSB=0x%x\n",avc_otp_data->awb_bg_msb);
			        	printk("[ov5670]read otp data AWB_LSB=0x%x\n",avc_otp_data->awb_lsb);
					#endif
					*sensor_id = imgsensor_info.sensor_id; 
					front_cam_id = OV5670AVCMIPI_SENSOR_ID;
					return ERROR_NONE;
				}
				else 
					  ...
					  MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	spin_unlock(&imgsensor_drv_lock);
//	slim_video_setting();
	hd_1080p_video_setting(); //m71 use 1080p setting
...
UINT32 OV5670AVC_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
