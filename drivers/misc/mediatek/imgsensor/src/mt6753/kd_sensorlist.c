...
extern int factory_fps;
extern int MoveAF_to_INF(int pos); //interface for factory test
#endif
/*******************************************************************************
*
********************************************************************************/
...
static struct device_attribute dev_attr_platform = {
	...
	.attr = {.name = "dev_platform", .mode = 0644},
	.show = platform_show,
...
static ssize_t camera_type_show(struct device *dev,
	...
	if(platform_match == 0)
		p += sprintf(p,"MZ_Project: UNKNOWN\n");
	else
		p += sprintf(p,"MZ_Project: MEIZU\n");
	
	if(back_cam_id == S5K3L2XX_SENSOR_ID)
		p += sprintf(p,"Back camera: 13M_PRI_Largen\n");
	else if(back_cam_id == S5K3L2XXGE_SENSOR_ID)
		p += sprintf(p,"Back camera: 13M_PRI_GESO\n");
	else if(back_cam_id == S5K3L2XXLITEON_SENSOR_ID)
		p += sprintf(p,"Back camera: 13M_LIT_Largen\n");
	else if(back_cam_id == S5K3L2XXLITEONGE_SENSOR_ID)
		p += sprintf(p,"Back camera: 13M_LIT_GESO\n");
	else if(back_cam_id == S5K3L2XXAVC_SENSOR_ID)
		p += sprintf(p,"Back camera: 13M_AVC\n");
	else 
		p += sprintf(p,"Unknow back camera\n");

	if(front_cam_id == OV5670MIPI_SENSOR_ID)
		p += sprintf(p,"Front camera: 5M_SUN\n");
	else if(front_cam_id == OV5670FILMMIPI_SENSOR_ID)
		p += sprintf(p,"Front camera: 5M_OFM\n");
	else if(front_cam_id == OV5670TECHMIPI_SENSOR_ID)
		p += sprintf(p,"Front camera: 5M_TECH\n");
	else if(front_cam_id == OV5670AVCMIPI_SENSOR_ID)
		p += sprintf(p,"Front camera: 5M_AVC\n");
	else 
		p += sprintf(p,"Unknow front camera\n");

	return (p - buf);
}

static ssize_t camera_type_store(struct device *dev,
		struct device_attribute *attr,char *buf,size_t count){

	return count;
}

static struct device_attribute dev_attr_camera_type = {

	.attr = {.name = "camera_type", .mode = 0644},
	.show = camera_type_show,
	.store= camera_type_store,
};

/*for test af infinite position*/
static ssize_t af_pos_show(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	return sprintf(buf,"inf:%d\nmacro:%d\n",af_inf_pos,af_macro_pos);
}
				...
				const char *buf, size_t count)
{
	...
	/* when infinite == 1, af will be pull to infinite,otherwise,pull to macro position */
	ret = MoveAF_to_INF(position);
...
static struct device_attribute dev_attr_fps = {

	.attr = {.name = "fps_test", .mode = 0644},
	.show = fps_show,
	.store= fps_store,
};

static ssize_t sn_info_show(struct device *dev,
				struct device_attribute *attr,
				char *buf)
{
	if(sn_info == 0)
		return sprintf(buf,"SN:NULL\n");
	else
		return sprintf(buf,"SN:%X\nYEAR:20%2d\n",sn_info,otp_year);
}

static ssize_t sn_info_store(struct device *dev,
				struct device_attribute *attr,
				const char *buf, size_t count)
{
	return count;
}
static struct device_attribute dev_attr_sn_info = {
	.attr = {.name = "sn_info", .mode = 0644},
	.show = sn_info_show,
	.store = sn_info_store,
};
#endif
...
#ifdef MEIZU_M81
    device_create_file(sensor_device,&dev_attr_platform);
    device_create_file(sensor_device,&dev_attr_camera_type);
    device_create_file(sensor_device,&dev_attr_af_pos);
    device_create_file(sensor_device,&dev_attr_fps);
    device_create_file(sensor_device,&dev_attr_sn_info);
#endif
    return 0;
}
