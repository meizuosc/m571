...
// otp slave address is equal to sub camera

#define SUNNY_MODULE_ID   0x01
#define OTP_DATA_LOG  	  0 

sunny_otp_struct *sunny_otp_data;

int read_otp(sunny_otp_struct *pOtp)
{

	int otp_flag, addr, temp,temp1, i;

	write_cmos_sensor(0x0100, 0x01);// must software stream on
	...
	if((otp_flag & 0xc0) == 0x40) {
		...
		addr = 0x701d; // base address of info group 3
	}
	if(addr != 0) {
		pOtp->flag = 0x80; // valid base info in OTP
		pOtp->module_id = read_cmos_sensor( addr );
		pOtp->lens_id = read_cmos_sensor( addr + 1);
		pOtp->IR_id = read_cmos_sensor( addr + 2);
		pOtp->production_year = read_cmos_sensor( addr + 3);
		pOtp->production_month = read_cmos_sensor( addr + 4);
		pOtp->production_day = read_cmos_sensor( addr + 5);
	}
	else {
		pOtp->flag = 0x00; // not info in OTP
		pOtp->module_id = 0;
		pOtp->lens_id = 0;
		pOtp->IR_id = 0;
		pOtp->production_year = 0;
		pOtp->production_month = 0;
		pOtp->production_day = 0;
	}
	// OTP WB Calibration
	 otp_flag = read_cmos_sensor(0x7023);
	
	 printk("[ov5670]=0x7020,awb_flag=0x%x\n",otp_flag);
	 addr = 0;
	 if((otp_flag & 0xc0) == 0x40) {
		 addr = 0x7024; // base address of WB Calibration group 1
 	}
 	else if((otp_flag & 0x30) == 0x10) {
 		addr = 0x7027; // base address of WB Calibration group 2
 	}
 	else if((otp_flag & 0x0c) == 0x04) {
 		addr = 0x702A; // base address of WB Calibration group 3
 	}
#if 0
	if(addr != 0) {
 		pOtp->flag |= 0x40;
 		temp = read_cmos_sensor( addr + 2);
 		pOtp->rg_ratio = (read_cmos_sensor(addr)<<2) + ((temp>>6) & 0x03);
 		pOtp->bg_ratio = (read_cmos_sensor( addr + 1)<<2) + ((temp>>4) & 0x03);
 	}
 	else {
 		pOtp->rg_ratio = 0;
 		pOtp->bg_ratio = 0;
	...
	if(addr != 0) {
 		pOtp->flag |= 0x40;
 		pOtp->awb_rg_msb = read_cmos_sensor(addr);
 		pOtp->awb_bg_msb = read_cmos_sensor(addr + 1);
 		pOtp->awb_lsb = read_cmos_sensor(addr + 2);
 	}
 	else {
 		pOtp->awb_rg_msb = 0; 
 		pOtp->awb_bg_msb = 0;
	...
	for(i=0x7010;i<=0x702C;i++) {
 		write_cmos_sensor(i,0); // clear OTP buffer, recommended use continuous write toaccelarate
 	}
	//set 0x5002[3] to “1”
 	temp1 = read_cmos_sensor(0x5002);
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == imgsensor_info.sensor_id) {				
				...
				sunny_otp_data = (sunny_otp_struct *)kzalloc(sizeof(sunny_otp_struct), GFP_KERNEL);
				if(NULL == sunny_otp_data) {
				        LOG_INF("allocate mem failed\n");
					return -ENOMEM;
				}
				int flag = read_otp(sunny_otp_data);
				if (sunny_otp_data->module_id == SUNNY_MODULE_ID)   //this driver is for sunny module
				{
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x,module id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id,sunny_otp_data->module_id);	  
			...
			#if OTP_DATA_LOG 
			printk("[ov5670]read otp data flag =0x%x\n",sunny_otp_data->flag);
			printk("[ov5670]read otp data module id =0x%x\n",sunny_otp_data->module_id);
			printk("[ov5670]read otp data IR id =0x%x\n",sunny_otp_data->IR_id);
			printk("[ov5670]read otp data lens id =0x%x\n",sunny_otp_data->lens_id);
			printk("[ov5670]read otp data AWB_RG_MSB=0x%x\n",sunny_otp_data->awb_rg_msb);
			printk("[ov5670]read otp data AWB_BG_MSB=0x%x\n",sunny_otp_data->awb_bg_msb);
			printk("[ov5670]read otp data AWB_LSB=0x%x\n",sunny_otp_data->awb_lsb);
			#endif
			}	
			retry--;
		} while(retry > 0);
