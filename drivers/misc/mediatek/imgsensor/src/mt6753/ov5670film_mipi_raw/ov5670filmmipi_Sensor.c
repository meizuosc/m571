...
#include "ov5670filmmipi_Sensor.h"
#include "meizu_otp.h"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670FILMMIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
ofilm_otp_struct *ofilm_otp_data;

static int read_otp(ofilm_otp_struct *pOtp)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == 0x5670) {				
				ofilm_otp_data = (ofilm_otp_struct *)kzalloc(sizeof(ofilm_otp_struct), GFP_KERNEL);
				if(NULL == ofilm_otp_data) {
				        LOG_INF("allocate mem failed\n");
					return -ENOMEM;
				}
				int flag = read_otp(ofilm_otp_data);
				if (ofilm_otp_data->module_id == OFILM_MODULE_ID)   //this driver is for ofilm module
				{	
					LOG_INF("i2c write id: 0x%x, sensor id: 0x%x,module id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id,ofilm_otp_data->module_id);	  
					#if OTP_DATA_LOG 
					printk("[ov5670]read otp data flag =0x%x\n",ofilm_otp_data->flag);
					printk("[ov5670]read otp data module id =0x%x\n",ofilm_otp_data->module_id);
					printk("[ov5670]read otp data lens id =0x%x\n",ofilm_otp_data->lens_id);
					printk("[ov5670]read otp data AWB_RG_MSB=0x%x\n",ofilm_otp_data->awb_rg_msb);
				   	printk("[ov5670]read otp data AWB_BG_MSB=0x%x\n",ofilm_otp_data->awb_bg_msb);
			        	printk("[ov5670]read otp data AWB_LSB=0x%x\n",ofilm_otp_data->awb_lsb);
					#endif
					*sensor_id = imgsensor_info.sensor_id; 
					front_cam_id = OV5670FILMMIPI_SENSOR_ID;
					return ERROR_NONE;
				}
				else 
...
UINT32 OV5670FILM_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
