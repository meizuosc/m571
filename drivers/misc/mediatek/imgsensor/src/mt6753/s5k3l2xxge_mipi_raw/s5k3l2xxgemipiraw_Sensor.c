...
#include "s5k3l2xxgemipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "S5K3L2XX_GESO_camera_sensor"
#define LOG_1 LOG_INF("S5K3L2XX_GESO,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = S5K3L2XXGE_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		if(module_id == PRIMAX_MODULE_ID){
			...
			if(program_id == 0x4D){
				...
				if(lens_id == 0x03){
                			LOG_INF("sensor id: 0x%x,pimax module id:%x,lens id:%x(GESO)\n",tmp,module_id,lens_id);
					*sensor_id = imgsensor_info.sensor_id;
					platform_match  = 1;
					back_cam_id = S5K3L2XXGE_SENSOR_ID;
					#if OTP_DATA_READ
    ...
    if (*sensor_id != imgsensor_info.sensor_id || module_id != PRIMAX_MODULE_ID || lens_id != 0x03) {
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == S5K3L2XX_SENSOR_ID && back_cam_id == S5K3L2XXGE_SENSOR_ID) {
                LOG_INF("s5k3l2xx primax module,GESO lens,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
...
UINT32 S5K3L2XXGE_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
