...
#include "s5k3l2xxavcmipiraw_Sensor.h"

/****************************Modify Following Strings for Debug****************************/
#define PFX "S5K3L2XXAVC_camera_sensor"
#define LOG_1 LOG_INF("S5K3L2XXAVC,MIPI 4LANE\n")
...
static imgsensor_info_struct imgsensor_info = {
    .sensor_id = S5K3L2XXAVC_SENSOR_ID,        //record sensor id defined in Kd_imgsensor.h
...
static avc_otp_struct *avc_otp_data;
u8 avc_lsc_data[AVC_LSC_SIZE];
...
static void read_otp_byte(u8 addr, u8 *buf,u8 write_id)
{
	...
	if (ret < 0)
		printk("read data from avc otp e2prom failed! error reg addr: 0x%X\n",pu_send_cmd[0]);
}
...
static void read_lsc_data(void){
	...
	for(ptr_addr = LSC_START_ADDR;ptr_addr < LSC_START_ADDR+AVC_LSC_SIZE;ptr_addr++){
		addr = ptr_addr%256;
		if( addr == 0) 
			write_id+=2;
		read_otp_byte(addr,&avc_lsc_data[ptr_addr-LSC_START_ADDR],write_id);
	}
}

static void read_otp_data(avc_otp_struct *pOtp)
{
	u8 addr = 0;
	u8 low = 0;
	u8 high = 0;
	...
	read_otp_byte(addr+40,&high,write_id); 
	read_otp_byte(addr+41,&low,write_id);
	pOtp->otp_af->af_infinity = high<<8|low;
	af_inf_pos = pOtp->otp_af->af_infinity;
	read_otp_byte(addr+42,&high,write_id);
	read_otp_byte(addr+43,&low,write_id); 
...
{	
	...
	write_cmos_sensor(0x0101, 0x00);        //add normal    
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		read_otp_byte(0x01,&module_id,E2PROM_WRITE_ID);
		if(module_id == AVC_MODULE_ID){
			read_otp_byte(0x02,&program_id,E2PROM_WRITE_ID);  //read 0x02 ,to confirm avc module
			read_otp_byte(0x06,&version_id,E2PROM_WRITE_ID);  //software version,0x01:mirror error 0x02:normal
			if(program_id == 0x4D && version_id == 0x01){
			LOG_INF("s5k3l2xx avc module,i2c write id: 0x%x, sensor id: 0x%x,avc module id:%x\n", imgsensor.i2c_write_id,tmp,module_id);
			#if OTP_DATA_READ
			//alloc memory for otp_data struct
			avc_otp_data = (avc_otp_struct *)kzalloc(sizeof(avc_otp_struct),GFP_KERNEL);
			avc_otp_data->otp_awb = (awb_struct *)kzalloc(sizeof(awb_struct),GFP_KERNEL);
			avc_otp_data->otp_af = (af_struct *)kzalloc(sizeof(af_struct),GFP_KERNEL);
			read_otp_data(avc_otp_data);
			#endif
			read_lsc_data();    
			*sensor_id = imgsensor_info.sensor_id;
			platform_match = 1;
			back_cam_id = S5K3L2XXAVC_SENSOR_ID;
			return ERROR_NONE;
			}
			else{
    ...
    if (*sensor_id != imgsensor_info.sensor_id || module_id != AVC_MODULE_ID || version_id != 0x01) {
...
static kal_uint32 open(void)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
        ...
        do {
            ...
            if (sensor_id == S5K3L2XX_SENSOR_ID && back_cam_id == S5K3L2XXAVC_SENSOR_ID) {
                LOG_INF("s5k3l2xx avc module,Largen lens,i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,sensor_id);
                break;
            }
...
UINT32 S5K3L2XXAVC_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
