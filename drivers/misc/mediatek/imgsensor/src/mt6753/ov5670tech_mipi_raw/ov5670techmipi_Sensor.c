...
#include "ov5670techmipi_Sensor.h"
#include "meizu_otp.h"
...
static imgsensor_info_struct imgsensor_info = { 
	.sensor_id = OV5670TECHMIPI_SENSOR_ID,		//record sensor id defined in Kd_imgsensor.h
...
#define OTECH_MODULE_ID   0x06
#define OTP_DATA_LOG  	  0


qtech_otp_struct *qtech_otp_data;

static int read_otp(qtech_otp_struct *pOtp)
{
	...
	if(addr != 0) {
		...
		pOtp->IR_id = read_cmos_sensor( addr + 1);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = ((read_cmos_sensor(0x300B) << 8) | read_cmos_sensor(0x300C));
			if (*sensor_id == 0x5670) {				
				qtech_otp_data = (qtech_otp_struct *)kzalloc(sizeof(qtech_otp_struct), GFP_KERNEL);
				if(NULL == qtech_otp_data) {
				        LOG_INF("allocate mem failed\n");
					return -ENOMEM;
				}
				int flag = read_otp(qtech_otp_data);
				if (qtech_otp_data->module_id == OTECH_MODULE_ID)   //this driver is for qtech module
				{	
					LOG_INF("i2c write id: 0x%x, sensor id: 0x%x,module id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id,qtech_otp_data->module_id);	  
					#if OTP_DATA_LOG 
					printk("[ov5670]read otp data flag =0x%x\n",qtech_otp_data->flag);
					printk("[ov5670]read otp data module id =0x%x\n",qtech_otp_data->module_id);
					printk("[ov5670]read otp data IR id =0x%x\n",qtech_otp_data->IR_id);
					printk("[ov5670]read otp data AWB_RG_MSB=0x%x\n",qtech_otp_data->awb_rg_msb);
				   	printk("[ov5670]read otp data AWB_BG_MSB=0x%x\n",qtech_otp_data->awb_bg_msb);
			        	printk("[ov5670]read otp data AWB_LSB=0x%x\n",qtech_otp_data->awb_lsb);
					#endif
					*sensor_id = imgsensor_info.sensor_id; 
					front_cam_id = OV5670TECHMIPI_SENSOR_ID;
					return ERROR_NONE;
				}
				else 
...
UINT32 OV5670TECH_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
