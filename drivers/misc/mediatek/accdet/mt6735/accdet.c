...
#ifdef MEIZU_M81
#define FIX_WORK_DELAY_TIME	500  /* delay 500ms and then run fix callback */
static struct delayed_work weird_headphone_fix_work;
static struct workqueue_struct *weird_headphone_fix_workqueue = NULL;
#endif

#ifdef MEIZU_M81
#define MEIZU_STATE_CHANGE_DELAY_REPORT
#endif

#ifdef MEIZU_STATE_CHANGE_DELAY_REPORT
#define ACCDET_STATE_DEBOUNCE_TIME			400
#define ACCDET_STATE_PLUG_OUT_DEBOUNCE_TIME	300
#define ACCDET_STATE_PLUG_IN_DEBOUNCE_TIME	400
static struct delayed_work accdet_state_change_delay_work;
static struct workqueue_struct *accdet_state_change_delay_workqueue = NULL;

struct wake_lock accdet_state_lock;
#endif
...
#ifdef MEIZU_STATE_CHANGE_DELAY_REPORT
	pr_info("[Accdet]%s(): pre_type(%d)\n", __func__, pre_type);
	if (pre_type == HEADSET_NO_MIC) {
		queue_delayed_work(accdet_state_change_delay_workqueue,
					&accdet_state_change_delay_work,
					msecs_to_jiffies(ACCDET_STATE_PLUG_OUT_DEBOUNCE_TIME));
	} else {
...
#ifdef MEIZU_M81
static void weird_headphone_fix(unsigned long a)
{
	pr_warn("[Accdet]%s(): fix weird headphone\n", __func__);
	switch_set_state((struct switch_dev *)&accdet_data, HEADSET_NO_MIC);
}
#endif
...
//Disable ACCDET
   ...
   //KE under fastly plug in and plug out
    ...
    if (cur_eint_state == EINT_PIN_PLUG_IN) {
		pr_info("[Accdet]EINT func :plug-in\n");	//ywl: ACCDET_DEBUG
#ifdef MEIZU_STATE_CHANGE_DELAY_REPORT
		cancel_delayed_work_sync(&accdet_state_change_delay_work);
#endif
		mutex_lock(&accdet_eint_irq_sync_mutex);
		eint_accdet_sync_flag = 1;
		mutex_unlock(&accdet_eint_irq_sync_mutex);
...
//EINT_PIN_PLUG_OUT
//Disable ACCDET
		pr_info("[Accdet]EINT func :plug-out\n");	//ywl: ACCDET_DEBUG
#ifdef MEIZU_STATE_CHANGE_DELAY_REPORT
		cancel_delayed_work_sync(&accdet_state_change_delay_work);
#endif
		mutex_lock(&accdet_eint_irq_sync_mutex);
		...
		del_timer_sync(&micbias_timer);
        #ifdef MEIZU_M81
            cancel_delayed_work_sync(&weird_headphone_fix_work);
        #endif
		#ifdef ACCDET_PIN_RECOGNIZATION
		  show_icon_delay = 0;
		  cable_pin_recognition = 0;
...
#ifdef MEIZU_STATE_CHANGE_DELAY_REPORT
static void accdet_state_change_delay_work_callback(struct work_struct *work)
{
	wake_lock(&accdet_state_lock);

	pr_info("[Accdet]%s(): eint_accdet_sync_flag(%d), cable_type(%d)\n",
			__func__, eint_accdet_sync_flag, cable_type);

	switch_set_state((struct switch_dev *)&accdet_data, cable_type);

	wake_unlock(&accdet_state_lock);
}
#endif
...
//#endif
	...
	*/
    ...
    #ifdef MEIZU_M81
		queue_delayed_work(weird_headphone_fix_workqueue,
					&weird_headphone_fix_work,
					msecs_to_jiffies(FIX_WORK_DELAY_TIME));
    #endif
	}
	#ifndef ACCDET_EINT_IRQ
...
/*
    MD         UP          DW
|---------|-----------|----------|
0V<=MD< 0.09V<= UP<0.17V<=DW <0.5V

*/

#define DW_KEY_HIGH_THR	 (500) //0.50v=500000uv
#define DW_KEY_THR		 (170) //0.17v=170000uv
...
#ifdef MEIZU_M81
	/* First cancel fix work and then run normal detect */
	pr_warn("[Accdet]cancel weird headphone fix work\n");
	cancel_delayed_work_sync(&weird_headphone_fix_work);

#ifdef MEIZU_STATE_CHANGE_DELAY_REPORT
	//ACCDET_DEBUG("[Accdet]%s(): cancel state change delay work\n", __func__);
	cancel_delayed_work_sync(&accdet_state_change_delay_work);
#endif
...
static void accdet_work_callback(struct work_struct *work)
{
    ...
    if(1 == eint_accdet_sync_flag) {
#ifdef MEIZU_STATE_CHANGE_DELAY_REPORT
		pr_info("[Accdet]%s():pre_status(%d), cable_type(%d)\n", __func__, pre_status, cable_type);
		if (/*pre_status == PLUG_OUT && */cable_type == HEADSET_NO_MIC) {
			queue_delayed_work(accdet_state_change_delay_workqueue,
					&accdet_state_change_delay_work,
					msecs_to_jiffies(ACCDET_STATE_PLUG_IN_DEBOUNCE_TIME));
		} else {
			switch_set_state((struct switch_dev *)&accdet_data, cable_type);
		}
#else
		...
		pr_warn("[Accdet] Headset has plugged out don't set accdet state\n");	//ywl: ACCDET_DEBUG
	}
	mutex_unlock(&accdet_eint_irq_sync_mutex);
...
int mt_accdet_probe(void)	
{
    ...
    #ifdef MEIZU_M81
        weird_headphone_fix_workqueue = create_singlethread_workqueue("weird_headphone_fix");
        INIT_DELAYED_WORK(&weird_headphone_fix_work, weird_headphone_fix);
    #endif

#ifdef MEIZU_STATE_CHANGE_DELAY_REPORT
		wake_lock_init(&accdet_state_lock, WAKE_LOCK_SUSPEND, "accdet state wakelock");

		accdet_state_change_delay_workqueue = create_singlethread_workqueue("accdet_state_delay");
		INIT_DELAYED_WORK(&accdet_state_change_delay_work, accdet_state_change_delay_work_callback);
#endif
...
void mt_accdet_remove(void)	
{
	...
	#endif
#ifdef MEIZU_STATE_CHANGE_DELAY_REPORT
	destroy_workqueue(accdet_state_change_delay_workqueue);
#endif
