	...
	if (IS_NET_ACTIVE(prAdapter, NETWORK_TYPE_P2P_INDEX) && 
        ...
        *pucPrimaryChannel = prP2PBssInfo->ucPrimaryChannel;
		...
		if (nicFreq2ChannelNum(prWifiVar->rConnSettings.u4FreqInKHz*1000) != *pucPrimaryChannel) {
			DBGLOG(CNM, INFO, ("p2p is running on Channel %d, but supplicant"
								" try to run as MCC\n", *pucPrimaryChannel));
			return FALSE;
		}
#endif
		DBGLOG(CNM, INFO, ("p2p is running on Channel %d, supplicant"
							" try to run as SCC\n", *pucPrimaryChannel));
        return TRUE;
	}
#endif
    ...
    )
{
    ...
    if (prAisFsmInfo->prTargetBssDesc != NULL)
        printk("<samp> cnmBss40mBwPermitted: %s %d \n",
                prAisFsmInfo->prTargetBssDesc->aucSSID,
		prAisFsmInfo->prTargetBssDesc->eBand);

    if ((prAisFsmInfo->prTargetBssDesc != NULL) &&
            (prAisFsmInfo->prTargetBssDesc->eBand == BAND_2G4) &&
            (prAisFsmInfo->prTargetBssDesc->fgIsRalinkAp == TRUE)){
