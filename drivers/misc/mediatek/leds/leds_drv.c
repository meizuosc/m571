			    ...
			    unsigned long *delay_on, unsigned long *delay_off)
{
        ...
        if (led_data->cust.data == MT65XX_LED_PMIC_NLED_ISINK0)
        {
                /*
                 * if led_data->delay_on=0, led_data->delay_off=0
                 * then set delay_on=delay_off=1
                 * In order to enable breath mode, LED blink when "echo timer > /sys/class/leds/button-backlight/trigger"
		 *
		 * After invoking led_switch_breath_pmic, set back "led_data->delay_on=0, led_data->delay_off=0"
                 */
...
#ifdef MEIZU_M81
static ssize_t store_oneshot_blink(struct device *dev,struct device_attribute *attr, const char *buf, size_t size)
{
	unsigned int enable_oneshot = 0;

	if (buf != NULL && size != 0)
	{
		enable_oneshot  = (unsigned int)simple_strtoul(buf, NULL, 10);
		LEDS_DRV_DEBUG("store_oneshot_blink: enable:%u, size:%zu, address:0x%s\n", enable_oneshot, size, buf);

		if (enable_oneshot)
		{
			/* Schedule LED oneshot work */
			schedule_work(&oneshot_work);
		}
	}
	return size;
}

static DEVICE_ATTR(oneshot_blink, 0220, NULL, store_oneshot_blink);
#endif //end #ifdef MEIZU_M81
...
#endif //end #ifndef MEIZU_M81
{
	...
	for (i = 0; i < MT65XX_LED_TYPE_TOTAL; i++) {
...
#ifdef MEIZU_M81
		if (strcmp(g_leds_data[i]->cdev.name,"button-backlight") == 0)
			INIT_WORK(&oneshot_work, mz_led_oneshot_work);
#endif
		...
		if(strcmp(g_leds_data[i]->cdev.name,"button-backlight") == 0)
		{
			rc = device_create_file(g_leds_data[i]->cdev.dev, &dev_attr_oneshot_blink);
			if (rc)
				LEDS_DRV_DEBUG("[LED]device_create_file oneshot blink fail!\n");
		}
#endif //end #ifdef MEIZU_M81
