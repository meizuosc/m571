...
#define MZ_SELF_TEST
#ifdef MZ_SELF_TEST
extern s32 gtp_test_sysfs_init(struct kobject *);
...
static u8 gt9xx_tp_state = MZ_GESTURE_PART;
static u8 gt9xx_tp_state_pre = MZ_GESTURE_PART;
...
#ifdef MZ_HALL_MODE
static u8 config_hall_mode[GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH]
    = {GTP_REG_CONFIG_DATA >> 8, GTP_REG_CONFIG_DATA & 0xff};
#endif
                               ...
                               unsigned long arg)
{
    ...
    if (err)
    {
        GTP_ERROR("tpd: access error: %08X, (%2d, %2d)\n", cmd, _IOC_DIR(cmd), _IOC_SIZE(cmd));
        return -EFAULT;
    }
    ...
    switch (cmd)
    {
        ...
        default:
            GTP_ERROR("tpd: unknown IOCTL: 0x%08x\n", cmd);
            err = -ENOIOCTLCMD;
            break;
...
void gt9xx_gesture_handler(u8 state)
{
	gt9xx_tp_state = state;
}
...
s32 gtp_send_cfg(struct i2c_client *client)
{
    ...
    // delay 50ms, to make sure cfg switch successfully
    msleep(50);
    return ret;
}
...
#ifdef MZ_HALL_MODE
    u8 cfg_info_hall_mode[] = CTP_CFG_HALL_MODE;
#endif
...
#ifdef MZ_HALL_MODE
    memset(&config_hall_mode[GTP_ADDR_LENGTH], 0, GTP_CONFIG_MAX_LENGTH);
    memcpy(&config_hall_mode[GTP_ADDR_LENGTH],
		    cfg_info_hall_mode, CFG_GROUP_LEN(cfg_info_hall_mode));
#endif
...
#ifdef MZ_HALL_MODE
static int gt9xx_hall_mode = 0;

static s32 gtp_send_cfg_hall_mode(struct i2c_client *client)
{
    s32 ret = 1;
    ...
    for (retry = 0; retry < 5; retry++)
    {
        ret = gtp_i2c_write(client, config_hall_mode, GTP_CONFIG_MAX_LENGTH + GTP_ADDR_LENGTH);
...
static ssize_t gtp_sysfs_hall_mode_show(struct device *dev,
		struct device_attribute *attr, char *buf)
{
	return sprintf(buf, "%d\n", gt9xx_hall_mode);
}

static ssize_t gtp_sysfs_hall_mode_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t count)
{
	int state = simple_strtol(buf, NULL, 10);

	gt9xx_hall_mode = !!state;
	if(gt9xx_hall_mode)
		gtp_send_cfg_hall_mode(i2c_client_point);
	else
		gtp_send_cfg(i2c_client_point);
	return count;
}

static DEVICE_ATTR(hall_mode, S_IRUGO|S_IWUSR,
		gtp_sysfs_hall_mode_show, gtp_sysfs_hall_mode_store);
#endif
...
static s32 tpd_i2c_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
...
#ifdef MZ_SELF_TEST
    gtp_test_sysfs_init(&client->dev.kobj);
#endif

#ifdef MZ_HALL_MODE
    sysfs_create_file(&client->dev.kobj, &dev_attr_hall_mode.attr);
#endif

    // keep this to make kernel compatible with old flyme
    ...
    mz_gesture_handle_register(gt9xx_gesture_handler);
...
static int touch_event_handler(void *unused)
{
	...
	mutex_lock(&gtp_suspend_lock);
        ...
        if (DOZE_ENABLED == doze_status)
        {
            ...
            {
		if(doze_buf[2] == 0xcc) {
			gtp_i2c_read(i2c_client_point, gesture_data, 6);
			if (((gesture_data[2]==1) || (gesture_data[2]==2) ||
			(gesture_data[2]==4)) && (gesture_data[3]==0) &&
			(gesture_data[4]==0) && (gesture_data[5]==0)) {
				//ignore double tap on HOME_KEY
		...
		GTP_DEBUG("0x814B = 0x%02X, gesture_code 0x%02x\n",
		...
		if(gesture_code != GESTURE_ERROR) {
			if(mz_gesture_report(tpd->dev, gesture_code))
	...
	if ((finger & 0x40) != 0)
	{
        ...
        if (key_value || pre_key)
        {
            ...
            if (!pre_touch)
            {
                ...
                {
			GTP_ERROR("report touch key %d, val %d\n",
					touch_key_array[i],
					key_value & (0x01 << i));
		    // record we have key pressed
		    if(key_value & (0x01 << i)) {
			set_bit(16 + i, &gt9xx_finger_status);
			continue;
		    } else if(test_bit(16 + i, &gt9xx_finger_status_pre)) {
			    GTP_ERROR("start report touch key, i %d", i);
			    input_report_key(tpd->dev, touch_key_array[i], 1);
			    input_report_key(tpd->dev, touch_key_array[i], 0);
			    //clear_bit(16 + i, &gt9xx_finger_status);
		    } else
			GTP_ERROR("ignore invalid key %d, val %d\n",
					touch_key_array[i],
					key_value & (0x01 << i));
                }
	    ...
	    if(gt9xx_finger_status != gt9xx_finger_status_pre) { // up non-contact finger
		    for(i = 0; i < GTP_MAX_TOUCH; i++)
			    if(!test_bit(i, &gt9xx_finger_status) &&
				test_bit(i, &gt9xx_finger_status_pre))
				    tpd_up(0, 0, i);
	    }
        }
        else
        {
			...
			tpd_up(0, 0, 0);
#endif
                GTP_DEBUG("Touch Release: %d, finger_status: %lx\n",
				pre_touch, gt9xx_finger_status_pre);
...
exit_free_dma_buffer:
    if (gpDMABuf_va) {
	dma_free_coherent(&tpd->dev->dev, GTP_DMA_MAX_TRANSACTION_LENGTH,
			gpDMABuf_va, gpDMABuf_pa);
	gpDMABuf_va = NULL;
	gpDMABuf_pa = 0;
	GTP_INFO("free DMA I2C buffer\n");
    }
    return -1;
}
...
#if GTP_GESTURE_WAKEUP
    // prevent tp_state changed during suspend time
    gt9xx_tp_state_pre = gt9xx_tp_state;
#ifdef MZ_HALL_MODE
    if ((gt9xx_tp_state_pre == MZ_GESTURE_DISABLE) ||
		    gt9xx_hall_mode) {
#else
    if (gt9xx_tp_state_pre == MZ_GESTURE_DISABLE) {
#endif
	pr_debug("gtp enter sleep mode\n");
	mz_set_tp_mode(MZ_SLEEP_MODE);
	mt_eint_mask(CUST_EINT_TOUCH_PANEL_NUM);
	ret = gtp_enter_sleep(i2c_client_point);
    } else {
	pr_debug("gtp enter gesture mode\n");
	mz_set_tp_mode(MZ_GESTURE_MODE);
...
#ifdef MZ_HALL_MODE
	if(gt9xx_hall_mode) {
		mz_set_tp_mode(MZ_COVER_MODE);
		gtp_send_cfg_hall_mode(i2c_client_point);
	} else
#endif
	{
		mz_set_tp_mode(MZ_HAND_MODE);
		gtp_send_cfg(i2c_client_point);
	}
    }
