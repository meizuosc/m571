...
static struct gtp_fw_info *gup_get_fw_info(void)
{
	int count = 0;

	while(count < sizeof(gtp_update_tb)) {
		if(!memcmp(gtp_update_tb[count].pid, update_msg.ic_fw_msg.pid,
					sizeof(update_msg.ic_fw_msg.pid))) {
			GTP_ERROR("match fw %d\n", count);
			return &gtp_update_tb[count];
		}
		count++;
	}

	return &gtp_update_tb[0]; //defualt 970
}
...
static u8 gup_get_ic_fw_msg(struct i2c_client *client)
{
    ...
    GTP_ERROR("IC force update flag:0x%x", update_msg.force_update);
...
static u8 gup_enter_update_judge(st_fw_head *fw_head)
{
    ...
    GTP_ERROR("FILE HARDWARE INFO:%02x%02x%02x%02x", fw_head->hw_info[0], fw_head->hw_info[1], fw_head->hw_info[2], fw_head->hw_info[3]);
    GTP_ERROR("FILE PID:%s", fw_head->pid);
    GTP_ERROR("FILE VID:%04x", fw_head->vid);
    GTP_ERROR("IC HARDWARE INFO:%02x%02x%02x%02x", update_msg.ic_fw_msg.hw_info[0], update_msg.ic_fw_msg.hw_info[1],
             update_msg.ic_fw_msg.hw_info[2], update_msg.ic_fw_msg.hw_info[3]);
    GTP_ERROR("IC PID:%s", update_msg.ic_fw_msg.pid);
    GTP_ERROR("IC VID:%04x", update_msg.ic_fw_msg.vid);
    ...
    GTP_ERROR("Firmware length:%d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);

    if (update_msg.fw_flag == 1)
	    return SUCCESS;
    else
	    update_msg.fw_flag = 1;

    if (update_msg.force_update != 0xBE)
    {
        GTP_ERROR("FW chksum error,need enter update.");
        return SUCCESS;
    }
    // 20130523 start
    if (strlen(update_msg.ic_fw_msg.pid) < 3)
    {
        GTP_ERROR("Illegal IC pid, need enter update");
        return SUCCESS;
    }
    else
    {
        for (i = 0; i < 3; i++)
        {
            ...
            {
                GTP_ERROR("Illegal IC pid, out of bound, need enter update");
                return SUCCESS;
            }
        }
    }
...
static u8 gup_check_update_file(struct i2c_client *client, st_fw_head *fw_head,
		u8 *path, struct gtp_fw_info *fw_ptr)
{
    s32 ret = 0;
    s32 i = 0;
    s32 fw_checksum = 0;
    ...
    if (path)
    {
        GTP_ERROR("Update File path:%s, %ld", path, strlen(path));
    ...
    else
    {
        ...
        GTP_ERROR("Update by default firmware array");
	if(fw_ptr == NULL)
		return FAIL;
        update_msg.fw_total_len = fw_ptr->length - FW_HEAD_LENGTH;
        if (fw_ptr->length < (FW_HEAD_LENGTH + FW_SECTION_LENGTH*4+FW_DSP_ISP_LENGTH+FW_DSP_LENGTH+FW_BOOT_LENGTH))
        {
            GTP_ERROR("INVALID fw header, check your gt9xx_firmware.h file!");
            return FAIL;
        }
        GTP_DEBUG("Firmware actual size: %d(%dK)", update_msg.fw_total_len, update_msg.fw_total_len/1024);
        memcpy(fw_head, fw_ptr->fwp, FW_HEAD_LENGTH);
        
        //check firmware legality
        fw_checksum = 0;
        for (i = 0; i < update_msg.fw_total_len; i += 2)
        {
            fw_checksum += (fw_ptr->fwp[FW_HEAD_LENGTH + i] << 8) + fw_ptr->fwp[FW_HEAD_LENGTH + i + 1];
        }
        
        GTP_ERROR("firmware checksum:%x", fw_checksum&0xFFFF);
...
static u8 gup_load_section_file(u8 *buf, u32 offset, u16 length, u8 set_or_end)
{
    ...
    if (HEADER_FW_READY == got_file_flag)
    {
        ...
        {
            memcpy(buf, &update_msg.fw_ptr->fwp[FW_HEAD_LENGTH + offset], length);
        }
        else    //seek end
        {
            memcpy(buf, &update_msg.fw_ptr->fwp[update_msg.fw_total_len + FW_HEAD_LENGTH - offset], length);
        }
        return SUCCESS;
    }
#endif
...
s32 gup_update_proc(void *dir)
{
    ...
    GTP_ERROR("[update_proc]Begin update ......");
    ...
    if (1 == searching_file)
    {
        ...
        GTP_ERROR("Exiting searching file for auto update.");
    ...
    update_msg.fw_ptr = gup_get_fw_info();
    if(update_msg.fw_ptr == NULL) {
	    GTP_ERROR("can not find fw header %s\n", update_msg.ic_fw_msg.pid);
	    goto file_fail;
    }

    update_msg.file = NULL;
    ret = gup_check_update_file(i2c_client_point, &fw_head, (u8*)dir, update_msg.fw_ptr);     //20121211
    if (FAIL == ret)
    {
    ...
    GTP_ERROR("firmware update begin...\n");
	...
	GTP_ERROR("gup_burn_dsp_isp success\n");
	...
	GTP_ERROR("gup_burn_fw_ss51 success\n");
	...
	GTP_ERROR("gup_burn_fw_dsp success\n");
        ...
        show_len = 50;
	GTP_ERROR("gup_burn_fw_boot success\n");
	...
	GTP_ERROR("gup_burn_fw_boot_isp success\n");
	...
	GTP_ERROR("gup_burn_fw_link success\n");
	...
	GTP_ERROR("gup_burn_fw_gwake success\n");
        show_len = 80;
        ...
        show_len = 90;
        GTP_ERROR("[update_proc]UPDATE SUCCESS.");
        retry = 0;
        break;
    }
...
update_fail:
    GTP_ERROR("[update_proc]leave update mode.");
    ...
    {
        GTP_ERROR("[update_proc]send config.");
